import { parsePayload } from "./featureFlagUtils.mjs";
...
function matchFeatureFlagProperty(property, propertyValues, options = {}) {
    ...
    const computeExactMatch = (target, actual)=>{
        if (Array.isArray(target)) return target.map((item)=>String(item).toLowerCase()).includes(String(actual).toLowerCase());
        ...
        case 'lte':
            {
                ...
                const parsedOverride = 'number' == typeof overrideValue ? overrideValue : null != overrideValue ? parseFloat(String(overrideValue)) : NaN;
...
function resolveFeatureFlagPayload(payloads, flagValue) {
    ...
    return null == payload ? null : parsePayload(payload);
}
export { InconclusiveMatchError, getFeatureFlagHash, getFeatureFlagVariant, getFeatureFlagVariantLookupTable, hashSHA1, matchFeatureFlagProperty, parseFeatureFlagSemver, relativeDateParseForFeatureFlagMatching, resolveFeatureFlagPayload };
