...
import { addGlobalEventListener, chain, hasFocusWithin, invariant, isFalsyBooleanCallback } from "@ariakit/utils";
...
function isMovingOnHovercard(path, card, hoverElements, nested) {
	...
	if (nested?.some((card) => hasFocusWithin(card) || path.includes(card))) return true;
	return false;
}
...
const useHovercard = createHook(function useHovercard({ store, modal = false, portal = modal, hideOnEscape = true, hideOnHoverOutside = true, disablePointerEventsOnApproach = !!hideOnHoverOutside, ...props }) {
	...
	const nestedHovercardsRef = useRef([]);
	...
	useEffect(() => {
		...
		const onMouseMove = (event) => {
			...
			const hoveredElement = hoverElements.find((element) => element && path.includes(element));
		...
		return chain(addGlobalEventListener("mousemove", onMouseMove, true), clearHideTimeout);
	}, [
		store,
		isMouseMoving,
		domReady,
