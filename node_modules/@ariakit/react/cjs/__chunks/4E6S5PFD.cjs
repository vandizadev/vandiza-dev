...
function useComputedStyle(store) {
  ...
  _TJ2QLEWIcjs.useSafeLayoutEffect.call(void 0, () => {
    ...
    const computedStyle = _TJ2QLEWIcjs.getWindow.call(void 0, contentElement).getComputedStyle(contentElement);
...
var ringLengthsRegex = /(?:^|\s)0(?:px)?\s+0(?:px)?\s+0(?:px)?\s+((?:\d*\.)?\d+)px(?=\s|$)/;
function getRingFromSegment(segment, maskedSegment) {
  ...
  if (!width) return;
  const lengthsStart = (_a = match.index) != null ? _a : 0;
...
var usePopoverArrow = _TJ2QLEWIcjs.createHook.call(void 0, function usePopoverArrow2({ store, size = defaultSize, borderWidth: borderWidthProp, ...props }) {
  const context = _UYZAI6KFcjs.usePopoverContext.call(void 0, );
  store = store || context;
  _TJ2QLEWIcjs.invariant.call(void 0, store, process.env.NODE_ENV !== "production" && "PopoverArrow must be wrapped in a Popover component.");
  const dir = _YNVZ54CPcjs.useStoreState.call(void 0, store, ["currentPlacement"], (state) => _FGIAPA3Ucjs.getBasePlacement.call(void 0, state.currentPlacement));
  const maskId = _TJ2QLEWIcjs.useId.call(void 0, );
  const style = useComputedStyle(store);
  const fill = (style == null ? void 0 : style.getPropertyValue("background-color")) || "none";
  const [borderWidth, isRing, ringColor] = _react.useMemo.call(void 0, () => {
  ...
  const fallbackColor = isRing ? style == null ? void 0 : style.getPropertyValue("color") : style == null ? void 0 : style.getPropertyValue(`border-${dir}-color`);
  ...
  props = {
    ...
    "aria-hidden": true,
    ...props,
    ref: _TJ2QLEWIcjs.useMergeRefs.call(void 0, store.setArrowElement, props.ref),
    style: {
      position: "absolute",
      fontSize: size,
      width: "1em",
...
var PopoverArrow = _TJ2QLEWIcjs.memo.call(void 0, _TJ2QLEWIcjs.forwardRef.call(void 0, function PopoverArrow2(props) {
