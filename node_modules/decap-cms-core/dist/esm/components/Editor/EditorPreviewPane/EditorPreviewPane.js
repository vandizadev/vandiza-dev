...
export class PreviewPane extends React.Component {
  getWidget = (field, value, metadata, props, idx = null) => {
     ...
     */
    return !widget.preview ? null : _jsx(PreviewHOC, {
    ...
    if (inferredField) {
      ...
      value = _jsxs("div", {
        children: [_jsxs("strong", {
          children: [field.get('label', field.get('name')), ":"]
        }), " ", value]
      });
    }
  ...
  widgetsFor = name => {
    ...
    if (List.isList(value) && variableTypes) {
      return value.map(val => {
        ...
        const widgets = typeFields && Map(typeFields.map((f, i) => [f.get('name'), _jsx("div", {
          children: this.getWidget(f, val, metadata.get(f.get('name')), this.props)
        }, i)]));
        return Map({
          data: val,
          widgets
    ...
    if (List.isList(value)) {
      return value.map(val => {
        const widgets = nestedFields && Map(nestedFields.map((f, i) => [f.get('name'), _jsx("div", {
          children: this.getWidget(f, val, metadata.get(f.get('name')), this.props)
        }, i)]));
        return Map({
          data: val,
  ...
  getCollection = async (collectionName, slugToLoad) => {
    ...
    const {
      ...
      slug,
      path
    } = await tryLoadEntry(state, selectedCollection, slugToLoad);
    return Map({
      data,
      slug,
      path
...
`;
    return _jsx(ErrorBoundary, {
      config: config,
      children: _jsx(PreviewPaneFrame, {
        id: "preview-pane",
        head: styleEls,
        initialContent: initialContent,
        children: _jsx(FrameContextConsumer, {
          children: ({
            document,
            window
          }) => {
            return _jsx(EditorPreviewContent, {
              previewComponent,
              previewProps: {
                ...previewProps,
                document,
